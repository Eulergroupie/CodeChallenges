...
//const toCamelCase = s => s.split(/[-_]/g).map((x,i)=>[...x].map((y,j)=>j==0&&i>0?y.toUpperCase():y)).flat().join('');
//const toCamelCase = s => s.split(/[-_]/g).map((x, i) => i > 0 ? x.replace(x[0],x[0].toUpperCase()) : x).join('');
